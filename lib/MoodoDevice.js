...
class MoodoDevice extends Homey.Device {
    ...
    onInit() {
        ...
        if (!this.hasCapability('fav')) this.addCapability('fav')
        if (!this.hasCapability('shuffles')) this.addCapability('shuffles')
        if (!this.hasCapability('slot0')) this.addCapability('slot0')
        if (!this.hasCapability('slot1')) this.addCapability('slot1')
        if (!this.hasCapability('slot2')) this.addCapability('slot2')
        if (!this.hasCapability('slot3')) this.addCapability('slot3')
    ...
    async refreshfavorites (){
        try{
            ...
            let mtoken = Homey.settings.get('moodotoken')
            let settings = this.getSettings()
            let devicee = this.getData() 
            let device = this
            let askdevice = {

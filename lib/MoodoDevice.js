...
class MoodoDevice extends Homey.Device {
    ...
    async refreshfavorites (){
        try{
            ...
            let values =[]
            let a       
            console.log("options: ",device.getCapabilityOptions("favs"))
            await device.removeCapability("favs")
            await device.removeCapability("mode_interval")
            await http.get(askdevice).then(function (result) {
                if (result.response.statusCode !== 200) return (new Error('no_devices'))
                if (result.data) {
                    for (a = 0; a < result.data.favorites.length; a++) {       
                    ...
                    device.setCapabilityOptions("favs", {
                        values: values
                    })
                    return 
                }
            ...
            await device.addCapability("favs")
            await device.addCapability("mode_interval")
    ...
    async FavoritesSettings(){
        try{
            let settings = this.getSettings()
            console.log(settings)
            if (settings.seefavorites === true){
                if (this.hasCapability("favs") === true){
                    console.log("ja la té")
                }else{
                    this.addCapability("favs")
                    console.log("afegida")
                }
            }else{
                this.removeCapability("favs")
                console.log("esborrada")
            }
        }catch (err) {
            throw new Error(err);   
        }
    ...
    async onSettings(oldsettings, newsettings, changed){
        try{
            ...
            //this._syncTimeout = setTimeout(this.FavoritesSettings.bind(this), (4000));
        }catch (err) {
            throw new Error(err);   
        }
    }
